...
void tile_drive_p_reset(uint8_t index){
	bos1921_write(index, BOS_REG_CONFIG, 0x0040);
//	HAL_Delay(50);
}

void tile_drive_p_set_mode(uint8_t index, uint8_t mode){
	switch(mode){
		default:
...
uint16_t tile_drive_p_read(uint8_t index){
	uint8_t RX_Buffer[2] = {0,0};
	int *rx_ptr = (int *)RX_Buffer;

	HAL_I2C_Mem_Read(drive_p_handle, (0x44+index)<<1, 0x00, 1, (uint8_t *)RX_Buffer, 2, 1000);

	return __builtin_bswap16(*rx_ptr);
}

void tile_drive_p_write(uint8_t index, uint8_t reg, uint16_t value){
	uint8_t TX_Buffer[2] = {0,0};
	int *tx_ptr = (int *)TX_Buffer;

	*tx_ptr = __builtin_bswap16(value);
	HAL_I2C_Mem_Write(drive_p_handle, (0x44+index)<<1, reg, 1, (uint8_t *)TX_Buffer, 2, 1000);
}
